...
async function pluginSettings() {
  ...
  let result = await prompt2({
    vrmUrl: {type:"textLine", label: "Upload a VRM file to <a href='//catbox.moe/'>catbox.moe</a> and put the URL here (url should end in <b>.vrm</b>). You can <b>make</b> a VRM character <a href='//vroid.com/en/studio'>here</a>, or you can download one <a href='//hub.vroid.com/en/models?is_other_users_available=1&is_downloadable=1'>here</a>.", defaultValue:oc.character.customData.vrmUrl || defaultVrmUrl},
    ...
    elevenLabsApiKey: {show:d=>d.voiceAudioEnabled=="yes", type:"textLine", label: "Enter your ElevenLabs API key (see <a href='//beta.elevenlabs.io/speech-synthesis'>user settings</a>):", defaultValue:oc.character.customData.elevenLabsApiKey || ""},
  ...
  for(let chunk of chunks) {
    ...
    if(chunk.text) {
      ...
      if(oc.character.customData.voiceAudioEnabled === "yes") {
        bufferPromise = fetch(`//api.elevenlabs.io/v1/text-to-speech/${oc.character.customData.elevenLabsVoiceId}?optimize_streaming_latency=0`, {
          method: "POST",
          headers: {
